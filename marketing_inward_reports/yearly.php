...
if ($selected_fy_id) {
    $sql = "
        ...
        WHERE mi.fiscal_year_id = :fy_id AND mi.status <> 'CANCELLED'
    ";
...
.filter-group select{padding:8px;border:1px solid #ddd;border-radius:4px;min-width:160px;font-size:12px;}
...
<h2>वार्षिक Marketing Inward विवरण (Yearly Marketing Inward Report)</h2>
...
<?php if (empty($records)): ?>
  <p style="text-align:center;color:#666;padding:30px;">No data found for this fiscal year.</p>
<?php else: ?>
...
function exportToCSV() {
    ...
    var a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = 'mi_yearly.csv';
...
function exportToExcel() {
    ...
    var a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = 'mi_yearly.xls';
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
}
</script>
