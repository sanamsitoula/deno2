...
// Validation
if (empty($book_code) || empty($ref_no) || empty($deno_date_nep) || empty($deno_date_eng) || 
    $per_poka_qty <= 0 || $poka_qty <= 0 || $logged_in_user_id <= 0) {
...
try {
    ...
    // Insert new record - created_by is automatically set to logged in user
    $stmt = $conn->prepare("
        INSERT INTO deno (
            book_code, ref_no, deno_date_nep, deno_date_eng,
            per_poka_qty, poka_qty, quantity_openpcs, notes,
            created_by, received_by, verify_by, update_remarks
        ) VALUES (
            :book_code, :ref_no, :deno_date_nep, :deno_date_eng,
            :per_poka_qty, :poka_qty, :quantity_openpcs, :notes,
            :created_by, :received_by, :verify_by, :update_remarks
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Deno record created successfully',
        'id' => $newId
    ]);
