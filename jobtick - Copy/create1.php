...
function generateJobTicketCode($conn, $fiscalYearId) {
    $stmt = $conn->prepare("SELECT fiscal_code FROM fiscal_years WHERE id = ?");
    $stmt->execute([$fiscalYearId]);
    $fyCode = $stmt->fetchColumn();
    
    $stmt = $conn->prepare("SELECT COUNT(*) FROM job_ticket WHERE fiscal_year_id = ?");
    $stmt->execute([$fiscalYearId]);
...
function getNextLotNumber($conn, $bookId, $fiscalYearId) {
    ...
    $stmt = $conn->prepare("
        SELECT COALESCE(MAX(CAST(lot AS INTEGER)), 0) as max_lot
        FROM job_ticket 
        WHERE book_id = ? AND fiscal_year_id = ?
    ");
    $stmt->execute([$bookId, $fiscalYearId]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    return ($result['max_lot'] ?? 0) + 1;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    error_log("Job Ticket Action: $action");
    ...
    try {
        switch ($action) {
            case 'create':
                ...
                error_log("Creating Job Ticket - Lot: $lotNumber, Code: $jobTicketCode");
                ...
                // Handle empty or null values for remarks and description
                ...
                error_log("Job Ticket Created with ID: $jobTicketId");
                ...
                if (isset($_POST['forma_id']) && is_array($_POST['forma_id'])) {
                    foreach ($_POST['forma_id'] as $index => $formaId) {
                        ...
                        $detailDescription = !empty($_POST['detail_description'][$index]) ? 
                                           trim($_POST['detail_description'][$index]) : null;
                        ...
                        error_log("Detail Row $index: " . print_r($detailParams, true));
                ...
                $conn->commit();
                error_log("Job Ticket Transaction Committed Successfully");
            ...
            case 'update':
                ...
                error_log("Updating Job Ticket ID: $jobTicketId");
                ...
                error_log("Deleted old details for Job Ticket ID: $jobTicketId");
                ...
                $conn->commit();
                error_log("Job Ticket Update Committed Successfully");
            ...
            case 'delete':
                ...
                $jobTicketId = $_POST['id'];
                error_log("Deleting Job Ticket ID: $jobTicketId");
                ...
                error_log("Job Ticket Deleted Successfully");
        ...
        error_log("ERROR in Job Ticket Operation: " . $e->getMessage());
...
<head>
    ...
    <title>Job Ticket Manager</title>
    <style>
    body {
        font-size: 16px;
...
<body>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="?edit_id=<?= $ticket['id'] ?>" class="btn btn-warning btn-sm">Edit</a>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('=== Job Ticket Manager Initialized ===');
        ...
        function calculateTotals() {
            ...
            console.log('Calculated Totals:', { totalPages, totalPrintQty });
        ...
        if (printQtyInput) {
            printQtyInput.addEventListener('input', function() {
                const mainQty = this.value.trim();
                console.log('Main Print Qty Changed:', mainQty);
                
                if (!mainQty || isNaN(mainQty) || mainQty <= 0) return;
        ...
        // Listen to changes in page and print qty inputs
        document.addEventListener('input', function(e) {
            if (e.target && (e.target.classList.contains('page-input') || e.target.classList.contains('detail-print-qty'))) {
                calculateTotals();
            }
        });
        ...
        document.addEventListener('change', function(e) {
            if (e.target && e.target.classList.contains('detail-forma')) {
                ...
                console.log('Forma Selected:', {
                    formaId: selectedOption.value,
                    formaName: selectedOption.text,
                    pageValue: pageValue
                });
        ...
        if (bookSearch) {
            bookSearch.addEventListener('focus', function() {
                console.log('Book search focused');
            ...
            bookOptions.addEventListener('click', function(e) {
                if (e.target.classList.contains('dropdown-option')) {
                    ...
                    const bookId = option.getAttribute('data-value');
                    const bookText = option.getAttribute('data-text');
                    const classLevel = option.getAttribute('data-class-level');
                    
                    console.log('Book Selected:', { bookId, bookText, classLevel });
                    
                    bookSearch.value = bookText;
                    bookIdInput.value = bookId;
                    classInput.value = classLevel;
                    bookOptions.style.display = 'none';
        ...
        function loadFormasAndLot() {
            ...
            console.log('Loading formas and lot for book ID:', bookId);
        ...
        function loadFormasForBook(bookId) {
            console.log('Fetching formas for book ID:', bookId);
            
            return new Promise((resolve, reject) => {
                fetch(`get_formas_with_details.php?book_id=${bookId}`)
                    .then(r => r.json())
                    .then(data => {
                        console.log('Formas API Response:', data);
                        
                        if (data.success) {
                            window.currentFormas = data.formas || [];
                            console.log('Loaded formas:', window.currentFormas);
                            
                            detailsTableBody.innerHTML = '';
                            
                            const mainPrintQty = printQtyInput.value || 100000;
                            
                            if (window.currentFormas.length > 0) {
                                window.currentFormas.forEach((forma, index) => {
                                    console.log(`Adding row for forma ${index + 1}:`, forma);
                                    addRowWithForma(forma, index + 1, mainPrintQty);
                                });
                            } else {
                                console.log('No formas found, adding empty row');
                                addEmptyRow(1);
                            }
                    ...
                    .catch(error => {
                        console.error('Error fetching formas:', error);
                        reject(error);
                    });
            });
        }
        ...
        function addRowWithForma(forma, orderNo, printQty) {
            console.log('Adding row with forma:', { forma, orderNo, printQty });
        ...
        function addEmptyRow(orderNo) {
            const mainPrintQty = printQtyInput.value || 100000;
            console.log('Adding empty row:', { orderNo, mainPrintQty });
        ...
        function loadLotInfo(bookId) {
            console.log('Fetching lot info for book ID:', bookId, 'Fiscal Year:', fiscalYearId);
            
            return fetch(`get_lot_info.php?book_id=${bookId}&fiscal_year_id=${fiscalYearId}`)
                .then(r => r.json())
                .then(data => {
                    console.log('Lot Info API Response:', data);
                    
                    if (data.success) {
                        lotInput.value = data.next_lot;
                        console.log('Next lot number:', data.next_lot);
                        
                        if (data.lot_history && data.lot_history.length > 0) {
                            console.log('Lot history:', data.lot_history);
                            let historyHtml = '';
                            ...
                            console.log('No lot history found');
                            lotHistory.style.display = 'none';
                        }
                    }
                })
                .catch(error => console.error('Error fetching lot info:', error));
        }
        ...
        function addRow() {
            ...
            console.log('Adding new row:', { rowCount, bookId });
            ...
            document.querySelectorAll('.remove-detail-row').forEach(btn => {
                btn.addEventListener('click', function() {
                    if (document.querySelectorAll('.detail-row').length > 1) {
                        console.log('Removing detail row');
        ...
        if (bookIdInput.value && <?= $edit_ticket ? 'true' : 'false' ?>) {
            console.log('Edit mode: Loading data for book ID:', bookIdInput.value);
            loadFormasAndLot();
        }
        ...
        if (jobTicketForm) {
            jobTicketForm.addEventListener('submit', function(e) {
                ...
                if (!bookIdInput.value) {
                    console.error('Validation error: No book selected');
                ...
                if (!hasForma) {
                    console.error('Validation error: No forma selected');
        ...
        console.log('=== Job Ticket Manager Ready ===');
    });
    </script>
