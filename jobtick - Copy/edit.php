...
function getLotHistory($conn, $bookId, $fiscalYearId) {
    $stmt = $conn->prepare("
        ...
        ORDER BY CAST(lot AS UNSIGNED) ASC
    ");
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('input', function(e) {
        if (e.target.classList.contains('print-qty-input') || e.target.classList.contains('page-input')) {
            calculateTotals();
        }
    });
    ...
    document.getElementById('addForma').addEventListener('click', function() {
        ...
        newRow.querySelectorAll('input[type="number"]').forEach(i => {
            ...
            else if (i.classList.contains('print-qty-input')) i.value = printQtyInput.value || '10000';
            else i.value = '0';
        });
