...
if ($report_type === 'mileage') {
    ...
    SELECT 
        ...
        /* Opening meter: first log's start_meter */
        (SELECT start_meter 
        ...
        /* Closing meter: last log's end_meter */
        (SELECT end_meter 
         FROM vehicle_daily_logs 
         WHERE vehicle_id = v.vehicle_id 
           AND fiscal_year = :fiscal_year 
        ...
        /* ============================================================
         * FIX: compute amounts as disburse_qty * rate_per_liter
         * The fuel_coupon_distributions table does NOT have a persisted
         * total_amount column — it must be calculated on the fly.
         * ============================================================ */

        /* Petrol qty */
        COALESCE((
            SELECT SUM(fcd.disburse_qty) 
        ...
        /* Petrol amount = qty × rate */
        COALESCE((
            SELECT SUM(fcd.disburse_qty * fcd.rate_per_liter) 
            FROM fuel_coupon_distributions fcd
            JOIN fuel_coupons fc ON fcd.coupon_id = fc.coupon_id
            WHERE fc.vehicle_id = v.vehicle_id
              AND fc.fiscal_year = :fiscal_year
        ...
        /* Grand total fuel amount = all fuel types combined */
        COALESCE((
            SELECT SUM(fcd.disburse_qty * fcd.rate_per_liter) 
            FROM fuel_coupon_distributions fcd
            JOIN fuel_coupons fc ON fcd.coupon_id = fc.coupon_id
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th colspan="6">खपत इन्धनको विवरण</th>
                    <th rowspan="3">जम्मा रकम<br>(रू.)</th>
                    <th rowspan="3">कैफियत</th>
                    <th rowspan="3">सम्बद्धीय सवारी<br>चालकको दस्तखत</th>
                </tr>
                <tr>
                    <th colspan="2">पेट्रोलको विवरण</th>
                    <th colspan="2">डिजलको विवरण</th>
            ...
            <tbody>
                ...
                <?php foreach ($data as $row): ?>
                    ...
                    <tr>
                        ...
                        <td class="amount"><?= $row['petrol_amount'] > 0 ? number_format($row['petrol_amount'], 2) : '-' ?></td>
                        
                        <!-- Diesel: Qty & Amount -->
                        <td><?= $row['diesel_qty'] > 0 ? number_format($row['diesel_qty'], 2) : '-' ?></td>
                        <td class="amount"><?= $row['diesel_amount'] > 0 ? number_format($row['diesel_amount'], 2) : '-' ?></td>
                        
                        <!-- Mobil: Qty & Amount -->
                        <td><?= $row['mobil_qty'] > 0 ? number_format($row['mobil_qty'], 2) : '-' ?></td>
                        <td class="amount"><?= $row['mobil_amount'] > 0 ? number_format($row['mobil_amount'], 2) : '-' ?></td>
                        
                        <!-- Total Amount -->
                        <td class="amount"><strong><?= number_format($row['total_fuel_amount'], 2) ?></strong></td>
                ...
                <tr class="total-row">
                    ...
                    <td class="amount"><strong><?= number_format($grand_total_petrol_amt, 2) ?></strong></td>
                    <td><?= $grand_total_diesel_qty > 0 ? number_format($grand_total_diesel_qty, 2) : '-' ?></td>
                    <td class="amount"><strong><?= number_format($grand_total_diesel_amt, 2) ?></strong></td>
                    <td><?= $grand_total_mobil_qty > 0 ? number_format($grand_total_mobil_qty, 2) : '-' ?></td>
                    <td class="amount"><strong><?= number_format($grand_total_mobil_amt, 2) ?></strong></td>
                    <td class="amount"><strong><?= number_format($grand_total_amount, 2) ?></strong></td>
                    <td colspan="2"></td>
                </tr>
            </tbody>
        </table>
