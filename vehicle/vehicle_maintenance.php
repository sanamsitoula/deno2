...
// FIXED: Query the base table directly with proper column references
...
// FIXED: Use a proper JOIN query instead of relying on the view
$stmt = $conn->prepare("
    SELECT 
        vmr.*,
        v.vehicle_no,
                    ...
                    <?php else: ?>
                        <?php foreach ($records as $record): ?>
                            <tr>
                                ...
                                <td class="no-print">
                                    <button onclick="printRecord(<?= $record['maintenance_id'] ?>)" 
                                            class="btn btn-primary btn-sm" style="margin-bottom: 5px;">🖨️ Print</button>
...
function printRecord(maintenanceId) {
    window.open('print_maintenance.php?id=' + maintenanceId, '_blank');
}
