...
/* ══════════════════════════════════════════════════
   FISCAL YEARS — single source of truth, read from
   the fiscal_years table (fiscal_name), not hardcoded.
   Used for the filter dropdown, the default filter
   value, the Reset link, and the Generate modal.
══════════════════════════════════════════════════ */
...
foreach ($fiscal_years_list as $fy) {
    if ($fy['is_active']) { $active_fiscal_year = $fy['fiscal_name']; break; }
}
if (!$active_fiscal_year && !empty($fiscal_years_list)) {
    $active_fiscal_year = $fiscal_years_list[0]['fiscal_name'];
}
    ...
    <div class="fgrid">
        <div class="fg"><label>Fiscal Year</label>
            <select name="f_fiscal" onchange="this.form.submit()">
                ...
                <option value="<?=htmlspecialchars($fy['fiscal_name'])?>" <?=$f_fiscal===$fy['fiscal_name']?'selected':''?>><?=htmlspecialchars($fy['fiscal_name'])?><?=$fy['is_active']?' (active)':''?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="fg" style="flex-direction:row;gap:5px">
            ...
            <a href="<?=$_SERVER['PHP_SELF']?>?f_fiscal=<?=urlencode($active_fiscal_year ?? '')?>" class="btn btn-sc">&#8635; Reset</a>
        </div>
    </div>
</form>
            ...
            <div class="fg2">
                <div class="fg"><label style="font-size:12px;font-weight:700;margin-bottom:3px">Fiscal Year</label>
                    <select name="fiscal_year" required>
                        <?php foreach($fiscal_years_list as $fy): ?>
                        <option value="<?=htmlspecialchars($fy['fiscal_name'])?>" <?=$fy['is_active']?'selected':''?>><?=htmlspecialchars($fy['fiscal_name'])?><?=$fy['is_active']?' (active)':''?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
