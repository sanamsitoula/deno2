...
/* ══════════════════════════════════════════════════
   CARRY-OVER METER: Opening meter of month N =
   closing meter of month N-1, per vehicle
══════════════════════════════════════════════════ */
foreach ($summary as $vid => &$months) {
    // Sort months by Nepali month order
    uksort($months, function($a, $b) use ($month_order) {
        return ($month_order[$a] ?? 99) - ($month_order[$b] ?? 99);
    });
    $prev_closing = null;
    foreach ($months as $mon => &$data) {
        if ($prev_closing !== null && ($data['opening_meter'] === null || $data['opening_meter'] == 0)) {
            // Only fill in if opening is missing/zero — don't override good data
            $data['opening_meter'] = $prev_closing;
        }
        if ($data['closing_meter'] !== null && $data['closing_meter'] > 0) {
            $prev_closing = $data['closing_meter'];
        }
    }
    unset($data);
}
...
═══════════════════════════════ */
@media print {
    @page { size:A4 landscape; margin:0.35in 0.3in; }
    ...
    table.main { font-size:8.5pt; page-break-inside:auto; }
    table.main th { background:#e0e0e0 !important; font-size:8pt; padding:3pt 3pt;
        -webkit-print-color-adjust:exact; print-color-adjust:exact; }
    table.main td { padding:3pt 3pt; border:0.5pt solid #999; font-size:8.5pt; }
    ...
    table.cdtbl { font-size:8pt; }
    table.cdtbl th { background:#e8e8e8 !important; padding:2pt 3pt; font-size:7.5pt; }
    table.cdtbl td { padding:2pt 3pt; border:0.5pt solid #ccc; font-size:8pt; }
    table.cdtbl .cdsub td { background:#e0f0e0 !important; }

    /* Maintenance table */
    .maint-section { box-shadow:none; page-break-before:always; }
    .maint-section h3 { font-size:10pt; padding:4pt 6pt; background:#e0e0e0 !important; print-color-adjust:exact; }
    table.maint { font-size:8.5pt; }
    table.maint th { background:#e0e0e0 !important; font-size:8pt; padding:3pt 3pt; print-color-adjust:exact; }
    table.maint td { padding:3pt 3pt; border:0.5pt solid #999; font-size:8.5pt; }
    .maint-tot td { background:#c8c8c8 !important; }

    /* Print-only elements — keep remarks + signature together with grand total, no page break */
    .remarks-box { display:block !important; margin-top:8pt; page-break-inside:avoid; page-break-before:avoid; }
    .sig-row      { display:block !important; margin-top:10pt; page-break-inside:avoid; page-break-before:avoid; }

    /* Wrap grand-total + remarks + signatures so they stay together */
    .footer-group { page-break-inside:avoid; }

    .rmk-tbl { width:100%; border-collapse:collapse; font-size:9pt; }
    .rmk-tbl td { border:0.5pt solid #888; padding:4pt 6pt; vertical-align:top; }

    /* Signature table */
    .sig-tbl { width:90%; margin:0 auto; border-collapse:collapse; font-size:9pt; }
    .sig-tbl td { text-align:center; padding:0 10pt; }
    .sigln { border-top:0.5pt solid #000; margin-top:28pt; padding-top:3pt; }
}
...
<!-- BEGIN footer-group: grand total footer + remarks + signatures stay together on print -->
