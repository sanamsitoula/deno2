...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    $export_query = "
        SELECT d.*, 
               ...
               (SELECT SUM(total_poka_qty) FROM d2m_items WHERE d2m_id = d.id) as total_poka,
               (
    ...
    if (!empty($search_params['fiscal_year'])) {
        $conditions .= " AND d.fiscal_year_id = :fiscal_year";
        $bind_params[':fiscal_year'] = $search_params['fiscal_year'];
    }
    if (!empty($search_params['book_code'])) {
        $conditions .= " AND di.book_code = :book_code";
    ...
    $export_query .= $conditions;
    $export_query .= " GROUP BY d.id, fy.fiscal_code, u_created.username, u_checked.username, u_verified.username";
    $export_query .= " ORDER BY d.nep_date DESC, d.created_at DESC";
    ...
    header('Content-Disposition: attachment;filename="D2M_Records_' . date('Y-m-d_His') . '.xls"');
    header('Cache-Control: max-age=0');
    
    echo '<html><head><meta charset="UTF-8"></head><body>';
    echo '<table border="1">';
    echo '<thead><tr style="background-color: #667eea; color: white; font-weight: bold;">';
    echo '<th>ID</th><th>D2M Number</th><th>Serial No</th><th>Type</th><th>Fiscal Year</th>';
    echo '<th>Nepali Date</th><th>English Date</th><th>Status</th><th>Items Count</th>';
    echo '<th>Total Poka</th><th>Total Books</th><th>DENO References</th>';
    echo '<th>Created By</th><th>Created At</th><th>Checked By</th><th>Checked At</th>';
    echo '<th>Verified By</th><th>Verified At</th><th>Remarks</th>';
    echo '</tr></thead><tbody>';
    
    foreach ($export_records as $record) {
        echo '<tr>';
        echo '<td>' . $record['id'] . '</td>';
        echo '<td>' . htmlspecialchars($record['d2m_no']) . '</td>';
        echo '<td>' . $record['serial_no'] . '</td>';
        echo '<td>' . ($record['d2m_type'] == 'T' ? 'Translated' : 'Non-Translated') . '</td>';
        echo '<td>' . htmlspecialchars($record['fiscal_year_name'] ?? 'N/A') . '</td>';
        echo '<td>' . htmlspecialchars($record['nep_date']) . '</td>';
        echo '<td>' . date('Y-m-d', strtotime($record['eng_date'])) . '</td>';
        echo '<td>' . $record['status'] . '</td>';
        echo '<td>' . number_format($record['items_count']) . '</td>';
        echo '<td>' . number_format($record['total_poka'] ?? 0) . '</td>';
        echo '<td>' . number_format($record['total_books']) . '</td>';
        echo '<td>' . htmlspecialchars($record['ref_numbers'] ?? 'N/A') . '</td>';
        echo '<td>' . htmlspecialchars($record['created_by_name']) . '</td>';
        echo '<td>' . date('Y-m-d H:i:s', strtotime($record['created_at'])) . '</td>';
        echo '<td>' . htmlspecialchars($record['checked_by_name'] ?? '-') . '</td>';
        echo '<td>' . ($record['checked_at'] ? date('Y-m-d H:i:s', strtotime($record['checked_at'])) : '-') . '</td>';
        echo '<td>' . htmlspecialchars($record['verified_by_name'] ?? '-') . '</td>';
        echo '<td>' . ($record['verified_at'] ? date('Y-m-d H:i:s', strtotime($record['verified_at'])) : '-') . '</td>';
...
if (isset($_GET['export']) && $_GET['export'] === 'pdf') {
    ...
    // Same query building as Excel export
    $search_params = [
        'd2m_no' => $_GET['d2m_no'] ?? '',
        'd2m_type' => $_GET['d2m_type'] ?? '',
        'status' => $_GET['status'] ?? '',
    ...
    $export_query .= $conditions;
    $export_query .= " GROUP BY d.id, fy.fiscal_code, u_created.username ORDER BY d.nep_date DESC";
    ...
    $html .= '<h2>D2M Records Report</h2>';
    $html .= '<p>Generated on: ' . date('F d, Y g:i A') . '</p>';
    $html .= '<p>Total Records: ' . count($export_records) . '</p>';
    $html .= '</div>';
    
    $html .= '<table><thead><tr>';
    $html .= '<th>ID</th><th>D2M Number</th><th>Type</th><th>Fiscal Year</th>';
    $html .= '<th>Date</th><th>Status</th><th>Items</th><th>Books</th><th>Created By</th>';
    $html .= '</tr></thead><tbody>';
    
    foreach ($export_records as $record) {
        $html .= '<tr>';
        $html .= '<td>' . $record['id'] . '</td>';
        $html .= '<td>' . htmlspecialchars($record['d2m_no']) . '</td>';
        $html .= '<td>' . ($record['d2m_type'] == 'T' ? 'T' : 'NT') . '</td>';
        $html .= '<td>' . htmlspecialchars($record['fiscal_year_name'] ?? 'N/A') . '</td>';
        $html .= '<td>' . htmlspecialchars($record['nep_date']) . '</td>';
        $html .= '<td>' . $record['status'] . '</td>';
        $html .= '<td>' . number_format($record['items_count']) . '</td>';
        $html .= '<td>' . number_format($record['total_books']) . '</td>';
        $html .= '<td>' . htmlspecialchars($record['created_by_name']) . '</td>';
        $html .= '</tr>';
    }
    ...
    try {
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output('D2M_Records_' . date('Y-m-d_His') . '.pdf', 'D');
    } catch (Exception $e) {
...
<div class="container">
    ...
    <?php if (isset($error_message)): ?>
        <div class="alert alert-danger">⚠️ <?= $error_message ?></div>
    <?php endif; ?>
        ...
        <div>
            <?php if (has_role('editor') || has_role('admin')): ?>
                <a href="create.php" class="btn btn-primary">
                    <i class="fas fa-plus-circle"></i> Create New D2M
                </a>
            <?php endif; ?>
        </div>
        ...
        <form method="get" id="searchForm">
            ...
            <div class="search-row">
                <div class="search-group">
                    ...
                    <input type="text" name="d2m_no" id="d2m_no" class="search-control" 
                    ...
                    <select name="book_code" id="book_code" class="search-control">
                        ...
                        <?php foreach ($book_codes as $book): ?>
                            <option value="<?= htmlspecialchars($book['book_code']) ?>"
                                    <?= $search_params['book_code'] == $book['book_code'] ? 'selected' : '' ?>>
                ...
                <div class="search-group">
                    ...
                    <select name="fiscal_year" id="fiscal_year" class="search-control">
                        <option value="">-- All Fiscal Years --</option>
                        <?php foreach ($fiscal_years as $fy): ?>
                            <option value="<?= $fy['id'] ?>"
                                    <?= $search_params['fiscal_year'] == $fy['id'] ? 'selected' : '' ?>>
            ...
            <div class="search-row">
                <div class="search-group">
                    ...
                    <input type="text" name="start_date" id="start_date" class="search-control" 
                           pattern="\d{4}\.\d{2}\.\d{2}" placeholder="2082.04.01"
                ...
                <div class="search-group">
                    ...
                    <input type="text" name="end_date" id="end_date" class="search-control" 
                           pattern="\d{4}\.\d{2}\.\d{2}" placeholder="2083.03.32"
    ...
    <div class="action-buttons">
        ...
        <div class="pagination-controls">
            ...
            <select onchange="window.location.href='?<?= http_build_query(array_merge($_GET, ['per_page' => '', 'page' => 1])) ?>&per_page=' + this.value">
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th style="width: 6%;">Checked By</th>
                    <th style="width: 6%;">Verified By</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="status-actions">
                                <!-- View Button -->
                                <a href="view.php?id=<?= $record['id'] ?>" class="btn btn-info btn-sm" title="View Details">
                                    <i class="fas fa-eye"></i> View
                                </a>
                                
                                <!-- Edit Button -->
                                <?php if ($record['status'] == 'DRAFT' && (has_role('editor') || has_role('admin'))): ?>
                                    <a href="edit.php?id=<?= $record['id'] ?>" class="btn btn-warning btn-sm" title="Edit">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                <?php endif; ?>
                                
                                <!-- Double Check Button -->
                                <a href="double_check.php?id=<?= $record['id'] ?>" class="btn btn-secondary btn-sm" title="Double Check">
                                    <i class="fas fa-check-double"></i> Check
                                </a>
                                
                                <!-- Print Button -->
                                <a href="print.php?id=<?= $record['id'] ?>" target="_blank" class="btn btn-primary btn-sm" title="Print">
                                    <i class="fas fa-print"></i>
                                </a>
                                
                                <!-- Mark as CHECKED -->
                                <?php if ((has_role('incharge') || has_role('operator') || has_role('supervisor') || has_role('admin')) && $record['status'] == 'DRAFT'): ?>
...
$(document).ready(function() {
    // Initialize Select2 for book code dropdown
    $('#book_code').select2({
