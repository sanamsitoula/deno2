...
// Fetch active fiscal years — fiscal_name stored in fiscal_year col, fiscal_code in deno_year col
$active_fiscal_years = $conn->query("
    SELECT fiscal_name, fiscal_code
    FROM   fiscal_years
    WHERE  is_active = true
    ORDER  BY fiscal_name
")->fetchAll(PDO::FETCH_ASSOC);

// Convenience: first active fiscal name/code for defaults
$active_fy_name = $active_fiscal_years[0]['fiscal_name'] ?? null;  // e.g. '2081-82'
$active_fy_code = $active_fiscal_years[0]['fiscal_code'] ?? null;  // e.g. '2082'
        ...
        switch ($action) {
            ...
            case 'update':
                ...
                if ($check->fetch()) {
                    ...
                    header('Location: ' . $_SERVER['PHP_SELF'] . '?edit_id=' . (int)$_POST['id']);
                    exit;
                }
...
$books = $conn->query("
    SELECT book_code, book_name FROM books ORDER BY book_name
")->fetchAll(PDO::FETCH_ASSOC);
...
.dropdown-options { position:absolute; top:100%; left:0; right:0; background:#fff; border:1px solid #ddd; border-top:none; max-height:200px; overflow-y:auto; z-index:1000; display:none; border-radius:0 0 4px 4px; }
...
<?php if (!empty($active_fiscal_years)): ?>
<div class="fy-info" style="margin-bottom:12px;">
    Active fiscal year(s):
    <?php foreach ($active_fiscal_years as $fy): ?>
        <span><?= htmlspecialchars($fy['fiscal_name']) ?></span>
        (code: <span><?= htmlspecialchars($fy['fiscal_code']) ?></span>)&nbsp;
    <?php endforeach; ?>
    — <em>deno_year &amp; fiscal_year are set automatically from the Nepali date.</em>
</div>
<?php endif; ?>
            ...
            <div class="search-dropdown">
                ...
                <div class="dropdown-options" id="book_options">
                    ...
                    <div class="dropdown-option"
                        ...
                        <small style="color:#888">(<?= htmlspecialchars($book['book_code']) ?>)</small>
                    </div>
                    <?php endforeach; ?>
                </div>
            </div>
...
<!-- Fiscal year lookup table from PHP (so JS can show live preview) -->
<script>
// fiscal years passed from PHP for client-side preview only
// The authoritative lookup is always done server-side by the trigger
var FISCAL_YEARS = <?php
    $fy_js = [];
    foreach ($active_fiscal_years as $fy) {
        $fy_js[] = [
            'fiscal_name' => $fy['fiscal_name'],
            'fiscal_code' => $fy['fiscal_code'],
        ];
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    <?php if ($edit_record): ?>
    (function () {
        var code = <?= json_encode($edit_record['book_code']) ?>;
        var el   = document.querySelector('.dropdown-option[data-value="' + code + '"]');
    ...
    function filterOptions() {
        ...
        options.forEach(function (o) {
            o.style.display = (o.textContent.toLowerCase().includes(t) || o.dataset.value.toLowerCase().includes(t))
                ? 'block' : 'none';
        });
    }
    options.forEach(function (o) {
    ...
    function updateFyDisplay(nepVal) {
        ...
        // Find matching entry in PHP-passed list
